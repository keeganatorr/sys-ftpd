...
#define STACK_SIZE 0x200000 //Change this if main-thread stack size ever changes.
...
void loadNro(void)
{
    ...
    fileStream = fopen ("sdmc:/.nxlink", "r");
    fscanf(fileStream,"%s\n%s",filePath,fileArgs);
    fclose(fileStream);

    char args[500];
    sprintf(args,"\"%s\" \"%s\"",filePath, fileArgs);
        
    if(filePath != NULL)
    {
        strcpy(g_nextNroPath, filePath);
        strcpy(g_nextArgv,    args);
    }
